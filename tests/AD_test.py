...
def test_tan():
    ...
    assert f.eval(x=1) == {'value': np.tan(1),
                           'derivative': 1 / ((np.cos(1)) ** 2)}, "Error: incorrect tangent function"
...
def test_func_comp():
    ...
    assert f2.eval(x=1) == {'value': np.exp(2 * np.sin(1)), 'derivative': 4 * np.exp(2 * np.sin(1)) * np.cos(
...
def test_constants():
    ...
    f = y + b
    results = f.eval(y=1)
    assert results['value'] == 5 and results['derivative'] == 1
    ...
    f = y * b
    results = f.eval(y=1)
    assert results['value'] == 4 and results['derivative'] == 4

    f = b ** y
    results = f.eval(y=2)
    assert results['value'] == 16 and np.isclose(results['derivative'], np.log(4) * 4 ** 2)

    f = y ** b
    results = f.eval(y=2)
    assert results['value'] == 16 and np.isclose(results['derivative'], 4 * (2 ** 3))
...
def test_divide():
    ...
    f = x / 4
    results = f.eval(x=4)
    assert results['value'] == 1 and np.isclose(results['derivative'] , 1/4)

    f = 4 / x
    results = f.eval(x=4)
    assert results['value'] == 1 and np.isclose(results['derivative'] , -4 / (4**2))
...
def test_subtraction():
    ...
    f = x - ad.const(4)
    results = f.eval(x=4)
    assert results['value'] == 0 and results['derivative'] == 1
...
def test_subtract_self():
    ...
    f = x - x
    results = f.eval(x=4)
    assert results['value'] == 0 and results['derivative'] == 0
...
def test_add_function():
    ...
    f = ad.add(x, 2)
    results = f.eval(x=4)
    assert results['value'] == 6 and results['derivative'] == 1

    f = ad.add(x, x)
    results = f.eval(x=4)
    assert results['value'] == 8 and results['derivative'] == 2
...
def test_self_divide():
    ...
    f = x / x
    results = f.eval(x=3)
    assert results['value'] == 1 and results['derivative'] == 0
